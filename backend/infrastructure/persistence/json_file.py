...
def _load_json(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        if isinstance(data, dict):
            return data
        logger.warning(
            "json_store_invalid_root_type",
            extra={"path": str(path), "root_type": type(data).__name__},
        )
        return {}
    except Exception as exc:
        quarantined_to = None
        try:
            quarantined_to = path.with_suffix(path.suffix + f".corrupted-{int(_utcnow().timestamp())}")
            path.replace(quarantined_to)
        except Exception as quarantine_exc:
            logger.error(
                "json_store_corrupted_file_quarantine_failed",
                extra={"path": str(path), "error": str(quarantine_exc)},
            )
        logger.error(
            "json_store_load_error",
            extra={
                "path": str(path),
                "error": str(exc),
    ...
    def _read_models(self) -> Dict[str, BaseModel]:
        ...
        for model_id, payload in raw.items():
            if not isinstance(payload, dict):
                logger.warning(
                    "json_store_invalid_model_payload",
                    extra={
                        "path": str(self._path),
                ...
                continue
            try:
                models[model_id] = self._model_type.model_validate(payload)
            except Exception as exc:
                logger.error(
                    "json_store_model_validation_error",
                    extra={
                        "path": str(self._path),
                        "model_type": self._model_type.__name__,
                        "model_id": model_id,
...
class JsonCheckpointStore:
    ...
    def save(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        checkpoint.setdefault("created_at", _utcnow().isoformat())
        with self._lock:
